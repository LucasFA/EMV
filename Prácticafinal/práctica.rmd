...
### Note
Notice that we received a warning about this being an Ultra-Heywood case. According to the literature, this means the model is not reliable. In short, it is a case in which the communalities exceed $1$. There could be several reasons for this.
...
Notice that some variables have a very pronounced mode in exactly the mean. This is because the mean is being used to replace the NA's. This is not a good idea, as it will skew the data. Instead, we will resample the data, and replace the NA's with the resampled values.
...
Still not normal at this point.
...
These distributions resemble the normal distribution more closely, so we can try to use the LDA and QDA models again.
...
We can make some transformations to the data. 
```{r}
...
We are finally succesful in transforming the data to a "more normal" distribution. Notice that the transformed variables have a much higher $p$-value in the shapiro test than they originally did.

In particular, let's compare the original and transformed data for the variables `plas` and `pedi`.
...
Unfortunately, these transformations do not seem to improve the models.
