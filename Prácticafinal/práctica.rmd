...
```{r}
# Print a histogram for each column giving the distribution of the class for each column
par(mfrow = c(2, 4))
for (k in seq_len(length(new_diab) - 1)) {
  variable_name <- names(new_diab)[k]
  for (i in 1:2) {
    output_res <- levels(new_diab$class)[i]
    x <- new_diab[new_diab$class == output_res, variable_name]
    hist(x,
      breaks = 20,
      main = paste("Histogram", variable_name, "for", output_res),
      xlab = variable_name,
      col = i + 1
    )
  }
...
for (k in seq_len(length(new_diab) - 1)) {
  ...
  for (i in 1:2) {
    level <- levels(new_diab$class)[i]
    x <- new_diab[new_diab$class == level, name_variable]
    qqnorm(x,
      main = paste("QQ-plot", name_variable, "for", level),
      pch = 19,
      col = i + 1)
    qqline(x)
  }
...
We previously saw that the data was not normally distributed with the Shapiro-Wilk test.
