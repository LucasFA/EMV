...
<!-- a) -->
We will first use the Bartlett test to check if the variables are homoscedastic. 
...
We reject the null hypothesis of homoscedasticity: the variables have different variances. 
...
#### Pipeline
```{r}
pipeline_model <- function(data, model_name, formula = class ~ plas + pres + skin + insu + mass + pedi + age) {
  set.seed(42)
...
### Linear Discriminant and cross-validation
...
### Quadratic Discriminant and cross-validation
...
Despite the fact that the data is not normally distributed, both the LDA and QDA models show a possible discriminant, albeit with a high error rate. 
...
The prior probabilities of belonging to `tested_negative` and `tested_positive` are  `0.6493` and `0.3506`, respectively, meaning the classes are imbalanced and `1 - 0.6493 = 0.3507` would be the expected error rate of the always positive classifier.

Having that in mind, our estimated error rate 24.5% of our classifier is almost a 10% improvement over that naive classifier, but leaves significant room for improvement.
