...
We reject the null hypothesis of uncorrelatedness, so applying PCA is justified.
...
## Factor analysis
### Preconditions
We saw in our previous analysis of correlations for PCA that the data was not uncorrelated according to the Bartlett test. We will now check for the factor analysis. We can visualize the correlation matrix:
...
Groupings are not very clear. Though there's a strong correlation between _skin_ and _mass_, and between _plas_ and _insu_.
...
We can compare the communalities:
```{r }
...
Notice that the communalities go beyond 1 in the minimum residual model. This is an ultra-Heywood case, which means that it is not reliable. We will therefore continue with the MLE model.

### Determine the number of factors
Again, we have a few methods to determine the number of factors to keep. We will use the scree plot and the parallel analysis.
...
### Note
Notice that we received a warning about this being an Ultra-Heywood case. According to the literature, this means the model is not reliable, and there could be several reasons for this.
...
There is a large number of outliers, as is noticeable.
...
Still very far from being normal distributions at this point.
...
We are finally succesful in transforming the data to a "more normal" distribution, at least to some degree.
...
The non-normality may contribute to the low performance of our classifier in this case. Even when transformed, the data does not follow the normal distribution.
The prior probabilities of belonging to `tested_negative` and `tested_positive` are  `0.6493` and `0.3506`, respectively, meaning the classes are imbalanced and `1 - 0.6493 = 0.3507` would be the expected error rate of the trivial, always positive classifier.

Having that in mind, our estimated error rate 24.5% of our classifier is almost a 10% improvement over that naive classifier.

## Conclusion

This dataset contains several variables that are not only not normally distributed, but have a high number of outliers because of it. Both of these factors may contribute to the low performance of our classifiers.