...
datos_pca <- lapply(datos_pca, function(x) not_available(x))
```
...
# write a function that removese outliers and replaces them NA
outlier <- function(x) {
  x[which(x > mean(x) + 3 * sd(x))] <- NA
  x[which(x < mean(x) - 3 * sd(x))] <- NA
  x
}
...
outlier <- function(data, na.rm = T) {
  H <- 1.5 * IQR(data)
  data[
      data < quantile(data, 0.25, na.rm) - H |
      data > quantile(data, 0.75, na.rm) + H
    ] <- NA
  data[is.na(data)] <- mean(data, na.rm = T)
  data
}
...
datos_pca <- lapply(datos_pca, outlier)
...
ggplot(
  data = data.frame(varianza_explicada, pc = 1:15),
...
ggplot(
  data = data.frame(varianza_acum, pc = 1:15),
